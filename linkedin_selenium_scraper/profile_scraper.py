...
class Descriptor:
    def __repr__(self):
        # TODO: fix indent
        trunc = lambda text, length: (text[:length] + '..') if \
            text and isinstance(text, str) and len(text) > length else text

        return '{} \n{}'.format(
            self.__class__.__name__, ' - '.
                join('     ( {}: {} )\n'.format(k, trunc(v, 22)) for k, v in self.__dict__.items()))
...
class LinkedinProfile(Descriptor):
  ...
  def __init__(self, url, email, password):
    ...
    profile.execute_script("window.scrollBy(0,500000)")
    ...
    ActionChains(profile).move_to_element(see_more).perform()
    ActionChains.click(see_more).perform()
